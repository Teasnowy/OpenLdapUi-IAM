<script setup lang="ts">
  ...
  // 接收值, upload为是否显示上传按钮
  let props = withDefaults(defineProps<{modelValue?:string|null, upload?:boolean}>(),{
    upload:()=>true
  })
  // 回传哪些值
  ...
  // 选择框中图片的url
  let imageUrl = ref(user_info.user_photo)
  // console.log("首次的base64: ", imageUrl.value)
  // 当前用户上传图片的对象(不包含预选图片)
  ...
  // 点击预选图片的操作, 替换当前图片
  async function btn_replace(path:string) {
    await fetch(path)
        .then(response => {
          if (!response.ok) {
    ...
    photo_list.value = []
    // console.log("预览的base64: ", imageUrl.value)
    emit('update:modelValue', data_request.user_photo_base64)
  }
  ...
  const handleChange: UploadProps['onChange'] = async (uploadFile, uploadFiles) => {
    ...
    // if (uploadFile.size! > 2048000) {
    //   global_window("error", "不可以上传大于2MB的图片")
    //   status_btn_upload.value = false
    //   return
    // }
    console.log(photo_list.value)
    // console.log("启用用户选择的图片: ", uploadFile.raw!.name)
    data_request.user_photo_base64 = await file_to_base64(uploadFile.raw!).catch(()=>{
      status_btn_upload.value = false
    }) as string || ""
    // 尝试压缩
    await compressAndCropBase64Image(data_request.user_photo_base64, 60).then((res)=>{
    ...
    // console.log("原图blob: ", uploadFile.raw)
  ...
  async function btn_upload() {
    ...
    // upload.value!.submit()
    status_btn_upload.value = true
    // 所以改为传输base64, 这里的转base64已经挪到了选择图片的时候
  ...
  <div>
    ...
    <div class="div_select">
      <el-upload
          ...
          v-loading="status_btn_upload"
      >
        <img v-if="imageUrl&&imageUrl!='undefined'" :src="imageUrl" class="avatar" alt="" v-loading="status_btn_upload"/>
